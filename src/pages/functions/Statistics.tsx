...
import { Copy, RotateCcw, BarChart3, TrendingUp, AlertTriangle, Star, StarOff } from "lucide-react";
import { useToast } from "@/hooks/use-toast";
import { BarChart, Bar, XAxis, YAxis, CartesianGrid, Tooltip, PieChart, Cell, Pie, ResponsiveContainer } from 'recharts';
...
export default function Statistics() {
  ...
  useEffect(() => {
    if (inputText) {
      localStorage.setItem("statistics-transformer", JSON.stringify({
        input: inputText,
        timestamp: Date.now()
      }));
    }
  ...
  const getDetailedStats = (text: string) => {
    ...
    const words = text.trim() ? text.trim().toLowerCase().match(/\b\w+\b/g) || [] : [];
    ...
    const positiveWords = ['good', 'great', 'excellent', 'amazing', 'wonderful', 'fantastic', 'awesome', 'love', 'like', 'happy', 'joy', 'success', 'perfect', 'brilliant'];
    const negativeWords = ['bad', 'terrible', 'awful', 'horrible', 'hate', 'dislike', 'sad', 'angry', 'fail', 'failure', 'wrong', 'problem', 'issue', 'difficult'];
    ...
    if (emails.length > 0) entities.push({ type: 'Email', count: emails.length, examples: emails.slice(0, 3) });
    if (urls.length > 0) entities.push({ type: 'URL', count: urls.length, examples: urls.slice(0, 3) });
    if (phones.length > 0) entities.push({ type: 'Phone', count: phones.length, examples: phones.slice(0, 3) });

    // Theme analysis (basic keyword extraction)
    const themes = mostCommonWords.slice(0, 5).map(([word, count]) => ({ theme: word, relevance: count }));

    // Alerts based on thresholds
    const alerts = [];
    if (wordCount > wordCountThreshold) {
      alerts.push({ type: 'warning', message: `Text exceeds ${wordCountThreshold} words (${wordCount} words)` });
    }
    if (readingTime > readingTimeThreshold) {
      alerts.push({ type: 'info', message: `Reading time exceeds ${readingTimeThreshold} minutes (${readingTime} minutes)` });
    }
    if (avgSentenceLength > 30) {
      alerts.push({ type: 'warning', message: 'Very long sentences detected - consider breaking them up' });
    }
    ...
    <div className="p-3 sm:p-6">
      ...
      <div className="mb-6">
        <div className="flex items-start justify-between mb-2">
          <h1 className="text-xl sm:text-2xl font-bold text-foreground">Text Analytics & Statistics</h1>
          <Button
            variant="ghost"
            size="icon"
        ...
        <p className="text-muted-foreground mb-4 text-sm sm:text-base">
          Comprehensive text analysis with statistics, sentiment analysis, visual charts, and customizable alerts.
        </p>
        
        <div className="bg-muted/50 rounded-lg p-3 border border-border">
          <p className="text-sm text-muted-foreground mb-1">Example:</p>
          <code className="text-xs sm:text-sm font-mono break-words">
            "This is a sample text for analysis. It contains multiple sentences and provides various statistics like word count, readability, and sentiment analysis."
          </code>
        </div>
      </div>
      ...
      {stats.alerts.length > 0 && (
        <div className="mb-6 space-y-2">
          {stats.alerts.map((alert, index) => (
            <Alert key={index} className={alert.type === 'warning' ? 'border-destructive/50' : 'border-primary/50'}>
          ...
          <CardHeader className="pb-3">
            <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
              ...
              <div className="flex gap-2">
                <Button size="sm" variant="outline" onClick={handleCopy} disabled={!inputText}>
                  <Copy className="w-3 h-3 mr-1" />
                  <span className="hidden sm:inline">Copy</span>
                </Button>
                <Button size="sm" variant="outline" onClick={handleReset} disabled={!inputText}>
          ...
          <CardContent className="p-3 sm:p-6">
            <Tabs defaultValue="basic" className="w-full">
                ...
                <div className="space-y-4">
                  ...
                  <div className="space-y-3 text-sm">
                    <div className="flex justify-between">
                      ...
                      <Badge variant={stats.readabilityScore === 'Very Easy' || stats.readabilityScore === 'Easy' ? 'secondary' : 'outline'}>
                        {stats.readabilityScore}
                      </Badge>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-muted-foreground">Sentiment:</span>
                      <Badge variant={stats.sentimentScore > 0 ? 'secondary' : stats.sentimentScore < 0 ? 'destructive' : 'outline'}>
                        {stats.sentimentScore > 0 ? 'Positive' : stats.sentimentScore < 0 ? 'Negative' : 'Neutral'} ({stats.sentimentScore})
                      </Badge>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-muted-foreground">Avg. words/sentence:</span>
                      <Badge variant="outline">{stats.avgWordsPerSentence}</Badge>
                    </div>
                    <div className="flex justify-between">
                ...
                {inputText ? (
                  <div className="space-y-6">
                    ...
                    {stats.mostCommonWords.length > 0 && (
                      <div>
                        <h4 className="text-sm font-medium mb-3">Word Frequency</h4>
                        <div className="h-48">
                          <ResponsiveContainer width="100%" height="100%">
                            <BarChart data={getChartData(stats.wordFrequency, 8)}>
                    ...
                    {stats.characterFrequency && stats.characterFrequency.size > 0 && (
                      <div>
                        <h4 className="text-sm font-medium mb-3">Character Distribution</h4>
                        <div className="h-48">
                          <ResponsiveContainer width="100%" height="100%">
                            <PieChart>
                              <Pie
                                data={getChartData(stats.characterFrequency, 6)}
                                cx="50%"
                                cy="50%"
                                outerRadius={60}
                              ...
                              >
                                {getChartData(stats.characterFrequency, 6).map((_, index) => (
              ...
              <TabsContent value="settings" className="mt-4 space-y-4">
                <div className="space-y-4">
                  <div className="space-y-2">
                    <Label htmlFor="word-threshold" className="text-sm font-medium">
                  ...
                  <div className="space-y-2">
                    <Label htmlFor="reading-threshold" className="text-sm font-medium">
                    ...
                    <Input
                      ...
                      onChange={(e) => setReadingTimeThreshold(Number(e.target.value))}
                      className="w-full"
                    />
                  </div>
