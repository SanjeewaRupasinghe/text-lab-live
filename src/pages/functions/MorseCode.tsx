...
import { Copy, RotateCcw, ArrowUpDown, Star, StarOff, Volume2, AlertCircle, CheckCircle, Radio } from "lucide-react";
...
export default function MorseCode() {
  ...
  useEffect(() => {
    if (inputText || outputText) {
      localStorage.setItem("morse-code-transformer", JSON.stringify({
        input: inputText,
        output: outputText,
        mode,
        timestamp: Date.now()
  ...
  const getStatusMessage = () => {
    ...
    switch (validationStatus) {
      case "valid":
        return mode === "encode" ? "Successfully encoded to Morse code" : "Valid Morse code decoded successfully";
      case "invalid":
    ...
    <div className="p-3 sm:p-6">
      ...
      <div className="mb-6">
        <div className="flex items-start justify-between mb-2">
          <h1 className="text-xl sm:text-2xl font-bold text-foreground">Morse Code Translator</h1>
          <Button
            variant="ghost"
            size="icon"
        ...
        <p className="text-muted-foreground mb-4 text-sm sm:text-base">
          Convert text to Morse code or decode Morse code back to text. Includes audio playback feature.
        </p>
        ...
        <Card>
          ...
          <CardContent className="p-3 sm:p-6">
            <textarea
              ...
              placeholder={mode === "encode" ? "Enter text to convert to Morse code..." : "Enter Morse code (use / for word separators)..."}
        ...
        <CardContent>
          <div className="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-6 gap-2 text-sm font-mono">
            {Object.entries(morseCode).slice(0, 26).map(([letter, code]) => (
              <div key={letter} className="flex justify-between p-2 bg-muted/50 rounded">
