...
import { Copy, RotateCcw, ArrowUpDown, Star, StarOff, AlertCircle, CheckCircle, Link } from "lucide-react";
...
export default function UrlEncoder() {
  ...
  useEffect(() => {
    ...
    if (saved) {
      try {
        const { input, output, mode: savedMode, encodingType: savedEncodingType, timestamp } = JSON.parse(saved);
  ...
  useEffect(() => {
    if (inputText || outputText) {
      localStorage.setItem("url-encoder-transformer", JSON.stringify({
        input: inputText,
        output: outputText,
        mode,
        encodingType,
  ...
  const getStatusMessage = () => {
    ...
    switch (validationStatus) {
      case "valid":
        return mode === "encode" ? "Successfully URL encoded" : "Successfully URL decoded";
      case "invalid":
    ...
    <div className="p-3 sm:p-6">
      ...
      <div className="mb-6">
        <div className="flex items-start justify-between mb-2">
          <h1 className="text-xl sm:text-2xl font-bold text-foreground">URL Encoder/Decoder</h1>
          <Button
            variant="ghost"
            size="icon"
        ...
        <p className="text-muted-foreground mb-4 text-sm sm:text-base">
          Encode URLs and URI components or decode URL-encoded strings. Choose between full URI or component encoding.
        </p>
        
        <div className="bg-muted/50 rounded-lg p-3 border border-border">
          <p className="text-sm text-muted-foreground mb-1">Example:</p>
          <code className="text-xs sm:text-sm font-mono break-words">
            "Hello World!" → "Hello%20World%21" (Component) or "Hello World!" → "Hello%20World!" (URI)
          </code>
        </div>
      </div>
      ...
      <div className="mb-6 space-y-4">
        ...
        {mode === "encode" && (
          <Tabs value={encodingType} onValueChange={(value) => setEncodingType(value as "component" | "uri")}>
            <TabsList>
